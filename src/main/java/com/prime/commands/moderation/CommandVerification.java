...
public class CommandVerification extends CommandHandler {
    ...
    public CommandVerification() {
        super(new String[]{"verification", "verify"}, CommandCategory.MODERATION, new PermsNeeded("command.verification", false, false), "Let you members accept rules before posting messages\n\nThis feature is partially inspired by [Flashbot](https://flashbot.de)", "setup\ndisable");
    }
    ...
    private Message disableVerification(CommandManager.ParsedCommandInvocation parsedCommandInvocation) {
        ...
        if (!Prime.getMySQL().verificationEnabled(message.getGuild())) {
        ...
        Prime.getMySQL().deleteGuildVerification(message.getGuild());
    ...
    public static void handleReaction(MessageReactionAddEvent event) {
        ...
        if (Prime.getMySQL().verificationEnabled(event.getGuild())) {
            ...
            if (event.getTextChannel().equals(channel)) {
                ...
                String emote = Prime.getMySQL().getVerificationValue(event.getGuild(), "emote");
                if (!emote.equals(event.getReactionEmote().getName()) && !emote.equals(event.getReactionEmote().getId()))
                    return;
                Role verfied = event.getGuild().getRoleById(Prime.getMySQL().getVerificationValue(event.getGuild(), "roleid"));
                ...
                message.editMessage(Prime.getMySQL().getVerificationValue(event.getGuild(), "verifiedtext").replace("%user%", event.getUser().getAsMention()).replace("%guild%", event.getGuild().getName())).queue();
                message.getReactions().forEach(r -> {
    ...
    public static void setupStepSix(Message message, Message response) {
        ...
        if (kicktime == 0) {
            ...
            setups.remove(message.getGuild());
            settingslist.remove(message.getGuild());
            Prime.getMySQL().createVerification(settings);
            return;
        }
