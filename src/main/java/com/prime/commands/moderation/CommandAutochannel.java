...
public class CommandAutochannel extends CommandHandler {
    ...
    protected Message execute(CommandManager.ParsedCommandInvocation parsedCommandInvocation, UserPermissions userPermissions) {
        ...
        JDA jda = Prime.getJDA();

        if (args.length == 1) {
            if (args[0].equalsIgnoreCase("list")) {
                String entry = Prime.getMySQL().getGuildValue(guild, "autochannels");
    ...
    private void createChannel(CommandManager.ParsedCommandInvocation parsedCommandInvocation) {
        ...
        String oldEntry = Prime.getMySQL().getGuildValue(guild, "autochannels");
        String newEntry = oldEntry + ", " + channel.getId();
        Prime.getMySQL().updateGuildValue(guild, "autochannels", newEntry);
    ...
    public static void deleteChannel(VoiceChannel channel, Message message, MessageReactionAddEvent event) {
        String oldEntry = Prime.getMySQL().getGuildValue(event.getGuild(), "autochannels");
        String newEntry = oldEntry.replace(channel.getId() + ",", "");
        Prime.getMySQL().updateGuildValue(event.getGuild(), "autochannels", newEntry);
