...
public class MusicReactionHandler {
    ...
    public synchronized boolean handle(long messageId, TextChannel channel, User invoker, MessageReaction.ReactionEmote emote, boolean isAdding) {
        ...
        RankSystem rank = discordBot.security.getRankSystem(invoker, channel);
    ...
    private void handleBanTrack(MusicPlayerHandler player, TextChannel channel, User invoker, RankSystem rank, boolean isAdding) {
        if (!isAdding || !rank.isAtLeast(RankSystem.BOT_ADMIN)) {
            return;
        }
    ...
    private void handleVoteSkip(MusicPlayerHandler player, TextChannel channel, User invoker, RankSystem rank, boolean isAdding) {
        if (!rank.isAtLeast(RankSystem.GUILD_ADMIN) && GuildSettings.getBoolFor(channel, GSetting.MUSIC_SKIP_ADMIN_ONLY)) {
            return;
        }
        if (isAdding) {
