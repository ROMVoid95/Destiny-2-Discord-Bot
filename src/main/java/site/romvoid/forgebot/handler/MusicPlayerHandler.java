...
import site.romvoid.forgebot.db.controllers.CBotPlayingOn;
import site.romvoid.forgebot.db.controllers.CGuild;
import site.romvoid.forgebot.db.controllers.CUser;
import site.romvoid.forgebot.db.model.OPlaylist;
import site.romvoid.forgebot.guildsettings.GSetting;
import site.romvoid.forgebot.handler.discord.MessageMetaData;
...
public class MusicPlayerHandler {
     ...
     * @return bool
     */
    public boolean canUseVoiceCommands(User user, RankSystem rank) {
        ...
        if (!"off".equals(messageType) && record.id > 0) {
            ...
            Consumer<Message> callback = (message) -> {
                ...
                if (PermissionUtil.checkPermission(message.getTextChannel(), guild.getSelfMember(), Permission.MESSAGE_ADD_REACTION, Permission.MESSAGE_HISTORY)) {
                    ...
                    if (aListenerIsAtLeast(RankSystem.BOT_ADMIN)) {
    ...
    public List<Member> getUsersInVoiceChannel() {
        ...
        if (currentChannel != null) {
            ...
            userList.addAll(connectedUsers.stream().filter(user -> !user.getUser().isBot() && !user.getVoiceState().isDeafened() &&
                    GuildSettings.get(currentChannel.getGuild()).canUseMusicCommands(user.getUser(), bot.security.getRankSystemForGuild(user.getUser(), currentChannel.getGuild()))
     ...
     * @return found a user?
     */
    public boolean aListenerIsAtLeast(RankSystem rank) {
        VoiceChannel currentChannel = bot.getJda().getGuildById(guildId).getAudioManager().getConnectedChannel();
        if (currentChannel != null) {
            for (Member member : currentChannel.getMembers()) {
                ...
                if (bot.security.getRankSystem(member.getUser()).isAtLeast(rank)) {
                    return true;
                }
            }
        }
